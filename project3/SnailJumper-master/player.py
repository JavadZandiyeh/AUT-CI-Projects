...
class Player(pygame.sprite.Sprite):
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        if len(obstacles) > 2:
            # x = np.array([math.fabs(obstacles[0]['x']-player_x)/screen_width, math.fabs(obstacles[0]['y']-player_y)/screen_height, math.fabs(obstacles[0]['y']-obstacles[1]['y'])/(math.fabs(obstacles[0]['x']-obstacles[1]['x'])+1)])
            x = np.array([math.fabs(obstacles[0]['x']-player_x)/screen_width, math.fabs(obstacles[0]['y']-player_y)/screen_height, math.fabs(obstacles[1]['x']-player_x)/screen_width, math.fabs(obstacles[1]['y']-player_y)/screen_height, math.fabs(obstacles[2]['x']-player_x)/screen_width, math.fabs(obstacles[2]['y']-player_y)/screen_height])
            x = x.reshape(6, 1)
            y = self.nn.forward(x)
            if y[0] >= 0.25:
